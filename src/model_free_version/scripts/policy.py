...
class TD3(object):
    ...
    def select_action(self, state):
        ...
        # print(final_observation_all.shape)
        ...
        lidar_ego_image = torch.from_numpy(lidar_ego_image).unsqueeze(1).float().to(device)
    ...
    def train(self, replay_buffer, batch_size=100):
        ...
        # for i in range(10):
        #     lidar_ego_image_all_out = Image.fromarray(np.uint8(lidar_ego_image[0][i]))
        #     lidar_ego_image_all_out.save('/home/cyx/my_gym_ros_rl/src/multi_agent_rl/image_ego/' + str(self.save_time).zfill(7) + '.jpg')
        #     self.save_time += 1
        ...
        next_lidar_ego_image = torch.from_numpy(next_lidar_ego_image).unsqueeze(1).float().to(device)
        ...
        t2_all = time.time()
        # print("Data Time : {}  ms".format(round(1000*(t2_all-t1_all),2)))
        ...
        with torch.no_grad():
            ...
            t2 = time.time()
            # print("Forward Actor Time : {}  ms".format(round(1000*(t2-t1),2)))
            ...
            t2 = time.time()
            # print("Forward Critic Time : {}  ms".format(round(1000*(t2-t1),2)))

        # Get current Q estimates
        t1 = time.time()
        current_Q1, current_Q2 = self.critic(lidar_ego_image, vel_ang, goal, action)
        t2 = time.time()
        # print("Forward Q Time : {}  ms".format(round(1000*(t2-t1),2)))
        ...
        t2 = time.time()
        # print("Critic Backward Time : {}  ms".format(round(1000*(t2-t1),2)))
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            t2 = time.time()
            # print("Actor Backward Time : {}  ms".format(round(1000*(t2-t1),2)))
...
def main():
    actor = Actor(3634, 2, 1).to(device)
    critic = Critic(3634, 2).to(device)
    test_input = torch.randn([2, 3634]).to(device)
    test_input_action = torch.randn([2, 2]).to(device)

    # summary(actor.cpu(),[np.array([3634])], device='cpu')
    # summary(critic.cpu(),[np.array([3634]),[2]], device='cpu')


    output = actor(test_input)
    output_critic = critic(test_input, test_input_action)
    # print(output)
    # print(output_critic[0].shape)
    # print(output_critic[1].shape)
