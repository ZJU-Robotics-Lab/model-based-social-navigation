...
class Transition_Model_Dataset(Dataset):
    def __init__(self, root, mode):
        ...
        if mode == 'training':
            self.indexes = self.indexes[:int(0.9 * max_length)]

        elif mode == 'validation':
            self.indexes= self.indexes[int(0.9 * max_length):int(1.0 * max_length)]

        else:
            self.indexes = self.indexes[int(1.0 * max_length):]
