...
from env_sample_four import EnvSampler

from environment_four_agent import Env
...
def train(env_sampler, predict_env, agent, env_pool, model_pool):
    ...
    for epoch_step in range(NUM_EPOCH):
        ...
        for i in count():
            ...
            #　真实环境step 填充真实环境数据集
            cur_state_1, next_state_1, action_1, reward_1, done_1, \
                cur_state_2, next_state_2, action_2, reward_2, done_2, \
                    cur_state_3, next_state_3, action_3, reward_3, done_3, \
                        cur_state_4, next_state_4, action_4, reward_4, done_4 \
                             = env_sampler.sample(agent)

            if flag_stop_1 == 0:
                env_pool.push(cur_state_1, next_state_1, action_1, reward_1, done_1)
            # if flag_stop_2 == 0:
            #     env_pool.push(cur_state_2, next_state_2, action_2, reward_2, done_2)
            # if flag_stop_3 == 0:
            #     env_pool.push(cur_state_3, next_state_3, action_3, reward_3, done_3)
            # if flag_stop_4 == 0:
            #     env_pool.push(cur_state_4, next_state_4, action_4, reward_4, done_4)
            ...
            if done_1 and flag_stop_1 == 0:
                flag_stop_1 = 1
            # if done_2 and flag_stop_2 == 0:
            #     flag_stop_2 = 1
            # if done_3 and flag_stop_3 == 0:
            #     flag_stop_3 = 1
            # if done_4 and flag_stop_4 == 0:
            #     flag_stop_4 = 1
            ...
            if done_1:
                ...
                if env_episode_num % 20 == 0 and env_episode_num != 0:
                    agent.save(weight_outdir + '/' + str(save_time))
                    # predict_env.model.save(world_models_weight_outdir + '/' + str(save_time))
                    save_time += 1
...
def train_policy_repeats(env_pool, model_pool, agent):
    ...
    for i in range(NUM_TRAIN_REPEAT):
        ...
        if model_batch_size > 0 and len(model_pool) > 0:
            env_state, env_next_state, env_action, env_reward, env_done = env_pool.sample(int(env_batch_size))
            ...
            model_reward = np.reshape(model_reward, (model_reward.shape[0]))
            model_done = np.reshape(model_done, (model_done.shape[0]))
            ...
            env_state, env_next_state, env_action, env_reward, env_done = env_pool.sample(BATCH_SIZE_POLICY)
